...
extension DashboardIPadTableViewController: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if(collectionView.tag == 1) {
            ...
            cell.iconImageView.setImageWith(URL(string: imageUrl), completed: { (image:UIImage?, error:Error?, _:SDImageCacheType, _:URL?) in
                if (error != nil) {
            ...
            if(collectionView.tag == 2) {
                ...
                if let imageURL = flexDeal.images.first?.url {
                    resortFlaxImageView.setImageWith(URL(string: imageURL), completed: { (image:UIImage?, error:Error?, _:SDImageCacheType, _:URL?) in
                        if (error != nil) {
