        ...
        extension GetawayAlertsIPhoneViewController: UITableViewDelegate {
            ...
            func tableView(_ tableView: UITableView, editActionsForRowAt indexPath: IndexPath) -> [UITableViewRowAction]? {
                ...
                if let alertStatus = getawayAlert.enabled {
                    ...
                    let latestCheckInDate = getawayAlert.latestCheckInDate?.dateFromString()
                    if alertStatus || (latestCheckInDate?.isLessThanDate(currentDate))! {
        ...
        extension GetawayAlertsIPhoneViewController: UITableViewDataSource {
            ...
            func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
                ...
                if !self.individualActivityIndicatorNeedToShow {
                    ...
                    if getawayAlert.enabled == false {
                        let currentDate = Date()
                        if let latestCheckInDate = getawayAlert.latestCheckInDate?.dateFromString() {
                            
                            if latestCheckInDate < currentDate {
                                cell.alertStatusButton.isHidden = false
                                cell.alertStatusButton.setTitle("Expired", for: .normal)
